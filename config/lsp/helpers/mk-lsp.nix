# From https://github.com/nix-community/nixvim/blob/53e85da2409ddfeef27e8cdc9c020433614329f9/plugins/lsp/language-servers/_mk-lsp.nix
# ########
# mkLsp #
#########
#
...
#
{ pkgs, ... }:
{ name, description ? "Enable ${name}.", serverName ? name
, package ? pkgs.${name}, url ? package.meta.homepage or null, cmd ? (cfg: null)
, settings ? (cfg: cfg), settingsOptions ? { }, extraConfig ? cfg: { }
, extraOptions ? { }, ... }:
# returns a module
{ pkgs, config, helpers, lib, ... }:
with lib;
let cfg = config.plugins.lsp.servers.${name};
in {
  meta.nixvimInfo = {
    # TODO: description
    inherit url;
    path = [ "plugins" "lsp" "servers" name ];
  };
  ...
  config = mkIf cfg.enable {
    ...
    plugins.lsp.enabledServers = [{
      ...
      extraOptions = {
        ...
        on_attach = helpers.ifNonNull' cfg.onAttach (helpers.mkRaw ''
          function(client, bufnr)
            ${
              optionalString (!cfg.onAttach.override)
              config.plugins.lsp.onAttach
            }
